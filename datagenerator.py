...
class ImageDataGenerator(object):
    ...
    def flow_from_list(self, x, y=None, directory=None, batch_size=32, shuffle=True, seed=None,
                target_size=(299,299), color_mode='color', interpolation='bilinear',
                save_to_dir=None, save_prefix='', save_format='png', subset=None):
        return ListIterator(
            x, y, self,
            directory=directory,
...
class ListIterator(Iterator):
    def __init__(self, x, y, image_data_generator, directory, target_size=(299, 299),
                 color_mode='color', batch_size=32, shuffle=False, seed=None,
                 data_format=None, interpolation='bilinear',
                 save_to_dir=None, save_prefix='', save_format='png',
                 subset=None):
        if y is not None and len(x) != len(y):
            raise ValueError('`x` (images tensor) and `y` (labels) '
        ...
        self.save_prefix = save_prefix
        self.save_format = save_format
        super(ListIterator, self).__init__(len(x), batch_size, shuffle, seed)
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        for i, j in enumerate(index_array):
            ...
            if grayscale:
                img = cv2.imread(os.path.join(self.directory, fname), cv2.IMREAD_GRAYSCALE)
            else:
                img = cv2.imread(os.path.join(self.directory, fname), cv2.IMREAD_COLOR)

            if self.target_size is not None:
                img = cv2.resize(img, self.target_size)
            if self.image_data_generator.preprocessing_function:
                img = self.image_data_generator.preprocessing_function(img.astype(K.floatx()))
